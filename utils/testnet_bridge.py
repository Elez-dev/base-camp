...
class TestnetBridge(Wallet):
    ...
    def bridge(self):
        ...
        if balance - Web3.to_wei(0.0003, 'ether') < Web3.to_wei(amount, 'ether'):
            amount = Web3.from_wei(balance - Web3.to_wei(0.0003, 'ether'), 'ether')
        ...
        txn = contract.functions.swapAndBridge(
            ...
            'value': value + Web3.to_wei(0.0003, 'ether'),
            **self.get_gas_price()
        })
